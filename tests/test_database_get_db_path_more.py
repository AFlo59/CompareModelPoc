...
def test_get_db_path_module_alias_preferred_when_equal():
    ...
    with patch.object(db, 'DB_PATH', Path('mod.db'), create=True), \
         patch.object(db.DatabaseConfig, 'DB_PATH', Path('mod.db')):
        p = db.get_db_path()
...
def test_get_db_path_config_preferred_when_different():
    ...
    db = importlib.reload(db)
    with patch.object(db, 'DB_PATH', Path('mod.db'), create=True), \
         patch.object(db.DatabaseConfig, 'DB_PATH', Path('cfg.db')):
        p = db.get_db_path()
