...
def create_memory_chart(memory_stats: Dict) -> go.Figure:
    ...
    fig.update_layout(
        title=f"Mémoire RAM - {memory_stats['memory_percent']:.1f}% utilisée",
        height=300,
        showlegend=True
    )
...
def show_system_monitoring():
    ...
    with col1:
        st.metric(
            "💻 CPU Global", 
            f"{cpu_stats['cpu_usage_total']:.1f}%",
            delta=None
        )
    ...
    with col4:
        st.metric(
            ...
            delta=f"{process_info['num_threads']} threads"
        )
    ...
    with st.expander("🔧 Détails CPU par coeur", expanded=False):
        ...
        fig = px.bar(cpu_cores_df, x='Coeur', y='Utilisation (%)', 
                     color='Utilisation (%)', 
