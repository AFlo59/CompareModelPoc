...
def create_campaign(user_id: int, name: str, themes: List[str], language: str, gm_portrait: Optional[str] = None) -> int:
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute('''INSERT INTO campaigns (user_id, name, themes, language, gm_portrait)
                     VALUES (?, ?, ?, ?, ?)''', (user_id, name, themes_str, language, gm_portrait))
...
def get_user_model_choice(user_id: int) -> str:
    ...
    with get_db_connection() as conn:
        cursor = conn.execute(
            "SELECT model FROM model_choices WHERE user_id = ? ORDER BY rowid DESC LIMIT 1",
            (user_id,)
        )
