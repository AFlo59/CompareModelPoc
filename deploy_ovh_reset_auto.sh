#!/bin/bash
# Script de mise à jour VPS OVH avec RESET automatique (sans confirmation interactive)
...
echo "🚀 Mise à jour VPS avec RESET automatique - $TIMESTAMP"
echo "📍 Serveur: $VPS_IP"
echo "👤 Utilisateur: $VPS_USER"
echo ""
echo "🔥 RESET AUTOMATIQUE EN COURS - BASE DE DONNÉES SERA SUPPRIMÉE !"
echo ""

# 1. Créer le nouveau package localement
echo "📦 Création du package de déploiement..."

# Activer le venv et utiliser python (compatible Windows bash)
if [ -f "venv/Scripts/python.exe" ]; then
    # Windows venv
    ./venv/Scripts/python.exe scripts/deploy.py package
elif [ -f "venv/bin/python" ]; then
    # Linux/Mac venv
    ./venv/bin/python scripts/deploy.py package
else
    # Fallback
...
mkdir -p ~/backups
echo "\$(date): Reset complet effectué par deploy_ovh_reset_auto.sh - TIMESTAMP=${TIMESTAMP}" >> ~/backups/reset_log.txt
...
if docker ps | grep -q docker_app_1; then
    ...
    if curl -f -s http://localhost:8501/_stcore/health > /dev/null 2>&1; then
        ...
        echo "💡 Attendez quelques minutes et vérifiez http://51.210.243.134:8501"
    fi
